  ...
  return (
    <GlassCard className="p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[14px] font-semibold text-[#1F2340]">Today schedule</p>
          <p className="mt-1 text-[12px] font-medium text-[#8E95B5]">Real appointments for today</p>
        </div>
        <Calendar className="h-5 w-5 text-[#8E95B5]" />
      </div>
      ...
      <div className="mt-4 grid gap-2">
        {bookings.length > 0 ? (
          bookings.slice(0, 7).map((booking) => (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <span className="truncate font-semibold text-[#1F2340]">
                  {booking.appointmentTime} - {booking.customerName || booking.customerEmail}
                </span>
                <StatusBadge status={booking.status} />
              </div>
              <p className="truncate text-[12px] font-medium text-[#8E95B5]">
          ...
          <div className="rounded-2xl border border-dashed border-[#DDE3F5] bg-white/45 px-4 py-5 text-center">
            <p className="text-[13px] font-semibold text-[#1F2340]">No bookings today</p>
            <p className="mt-1 text-[12px] font-medium text-[#8E95B5]">Today&apos;s schedule is clear.</p>
          </div>
        )}
      </div>
    </GlassCard>
